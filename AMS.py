...
def create_gradient(canvas, width, height):
    ...
    transition_point = int(width * 0.3)  # Blue occupies 30% of the width

    for i in range(width):
        if i < transition_point:
            # White to Blue (first 30% of the width)
            red = 255 - int(255 * (i / transition_point))
            green = 255 - int(255 * (i / transition_point))
            blue = 255
        else:
            # Blue to Violet (remaining 70% of the width)
            i2 = i - transition_point
            red = int(128 * (i2 / (width - transition_point)))  # Red increases to 128
            green = 0  # Green stays at 0
            blue = 255

        # Format the color as a valid hex string
...
def manually_fill():
    ...
    canvas = tk.Canvas(sb, width=580, height=320)
    canvas.pack(fill="both", expand=True)

    # Call the gradient function to draw the background
    create_gradient(canvas, 580, 320)
    ...
    def fill_attendance():
        ...
        global subb
        subb=SUB_ENTRY.get()
        Subject = subb  # Assign the subject name to the Subject variable
        if not Subject:  # Check if the subject name is empty
            err_screen_for_subject()
            return
        DB_table_name = f"{Subject}_{date_for_DB}_{Hour}{Minute}{Second}"
        # DB_table_name = DB_table_name.replace(" ", "_").replace("-", "_").replace("/", "_")
        ...
        # Enclose the table name in backticks in the SQL query
        sql = f"""
        ...
        if subb=='':
            ...
            ENR = tk.Label(MFW, text="Enter Enrollment", width=15, height=2, fg="black", bg="blue2",
                           font=('times', 15, ' bold '))
            ENR.place(x=30, y=100)

            STU_NAME = tk.Label(MFW, text="Enter Student name", width=15, height=2, fg="black", bg="blue2",
                                font=('times', 15, ' bold '))
            ...
            def create_csv():
                ...
                csv_name=f"{BASE_PATH}/Attendance/Manually Attendance/{DB_table_name}.csv"
            ...
            def attf():
                try:
                    ...
                    if system == 'darwin':  # macOS
                        subprocess.Popen(['open', f"{BASE_PATH}/Attendance/Manually Attendance/"])
                    elif system == 'windows':  # Windows
                        subprocess.Popen(r'explorer /select,"{}"'.format(f"{BASE_PATH}/Attendance/Manually Attendance/"))
                    else:
    ...
    SUB_ENTRY = tk.Entry(sb, width=20, bg="white", fg="red", font=('times', 23, ' bold '))
    SUB_ENTRY.place(x=250, y=105)

    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="black", bg="deep pink", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    global cursor
                    connection = pymysql.connect( # Specify the port here
                    )
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                ...
                print("Inserting data into DB:", Id, aa, date, timeStamp)
                try:
                    cursor.execute(sql)  ##for create a table
                    cursor.execute(insert_data, VALUES)
                    connection.commit() 
                ...
                Notifica.configure(text=M, bg="Green", fg="black", width=33, font=('times', 15, 'bold'))
                Notifica.place(x=20, y=250)
    ...
    Notifica = tk.Label(windo, text="Attendance filled Successfully", bg="Green", fg="black", width=33,
                            height=2, font=('times', 15, 'bold'))

    def Attf():
        import subprocess
        # Use the macOS 'open' command instead of Windows 'explorer'
        system = platform.system().lower()
        if system == 'darwin':  # macOS
            subprocess.Popen(['open', f"{BASE_PATH}/Attendance/"])
        elif system == 'windows':  # Windows
            subprocess.Popen(r'explorer /select,"{}"'.format(f"{BASE_PATH}/Attendance/"))
        else:
            print("Unsupported operating system:", system)
    ...
    tx = tk.Entry(windo, width=20, bg="white", fg="red", font=('times', 23, ' bold '))
    tx.place(x=250, y=105)

    fill_a = tk.Button(windo, text="Fill Attendance", fg="black",command=Fillattendances, bg="deep pink", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def admin_panel():
    ...
    # Create a Canvas widget for the gradient background
    canvas = tk.Canvas(win, width=880, height=420)
    canvas.pack(fill="both", expand=True)

    # Call the gradient function to draw the background
    create_gradient(canvas, 880, 420)
    ...
    un = tk.Label(win, text="Enter username", width=15, height=2, fg="black", bg="blue2",
                   font=('times', 15, ' bold '))
    un.place(x=30, y=50)

    pw = tk.Label(win, text="Enter password", width=15, height=2, fg="black", bg="blue2",
                  font=('times', 15, ' bold '))
    pw.place(x=30, y=150)
...
def trainimg():
    ...
    try:
        ...
        if len(faces) == 0 or len(Id) == 0:  # Check if faces or Ids are empty
            raise ValueError("No valid images found in the TrainingImage folder.")
    except Exception as e:
...
message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bg=None,fg="White", width=50,
                   height=3, font=('Helvetica', 30, 'italic bold '))

message.place(x=200, y=20)

Notification = tk.Label(window, text="All things good", bg="#32cd32", fg="black", width=15,
                      height=3, font=('Helvetica', 17, 'bold'))

lbl = tk.Label(window, text="Enter Enrollment", width=20, height=2, fg="white", bg="#50336b", font=('Helvetica', 15, ' bold '))
lbl.place(x=200, y=210)
...
lbl2 = tk.Label(window, text="Enter Name", width=20, fg="white", bg="#50336b", height=2, font=('Helvetica', 15, ' bold '))
lbl2.place(x=200, y=310)

txt2 = tk.Entry(window, width=20, bg="white", fg="red", font=('times', 25, ' bold '))
txt2.place(x=550, y=310)

clearButton = tk.Button(window, text="Clear",command=clear,fg="black"  ,bg="#ff1493"  ,width=10  ,height=1 ,activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="black"  ,bg="#ff1493"  ,width=10 ,height=1, activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, text="Check Register students",command=admin_panel,fg="black"  ,bg="#3d3deb"  ,width=19 ,height=1, activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
AP.place(x=800, y=410)

takeImg = tk.Button(window, text="Take Images",command=take_img,fg="black"  ,bg="#4682b4"  ,width=20  ,height=3, activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images",command=trainimg,fg="black"  ,bg="#4682b4"  ,width=20  ,height=3, activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance",command=subjectchoose,fg="black"  ,bg="#4682b4"  ,width=20  ,height=3, activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill  ,fg="black"  ,bg="#32cd32"  ,width=20  ,height=3, activebackground = "#ff4500" ,font=('Helvetica', 15, ' bold '))
